...
from ipynbsrv.core.models import CollaborationGroupShare
...
def add_user_to_share_groups(sender, group, user, **kwargs):
    ...
    """
    if group is not None and hasattr(group, 'collaboration_group') \
            and user is not None and hasattr(user, 'backend_user'):
        for group_share in CollaborationGroupShare.objects.filter(group=group.collaboration_group):
...
@receiver(post_delete, sender=CollaborationGroupShare)
...
@receiver(post_save, sender=CollaborationGroupShare)
